<template>
  <div class="pb-16">
    ...
    <template v-else-if="vm">
      <!-- Banner: largura total do layout `profile` (sem max-w-7xl no main) -->
      <div class="relative mt-6 w-full overflow-hidden border-b border-zinc-800 bg-zinc-950">
        <!-- Altura alinhada à faixa “container” antiga; imagem em cover em toda a largura -->
        <div
        ...
        <div class="mt-8 space-y-6">
          ...
          >
            <div class="flex flex-col gap-6 sm:flex-row sm:items-center">
              <div class="shrink-0">
                <div
                  class="mx-auto h-28 w-28 overflow-hidden rounded-full border-2 border-zinc-700 bg-zinc-800 sm:mx-0 sm:h-32 sm:w-32"
                >
                  <img
                    v-if="vm.avatarUrl"
                    :src="vm.avatarUrl"
          ...
          <!-- Aviso -->
          <section class="w-full rounded-2xl border border-amber-500/25 bg-amber-500/5 p-5 text-sm leading-relaxed text-amber-100/95 md:p-6">
          ...
          <!-- Sobre -->
          <section v-if="vm.about" class="w-full rounded-2xl border border-zinc-800 bg-zinc-900/50 p-5 md:p-6">
          ...
          <!-- Galeria: um item por linha, clique abre lightbox -->
          ...
          >
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              class="mt-4 inline-flex w-full items-center justify-center gap-2 rounded-xl bg-[#25D366] px-5 py-3.5 text-sm font-semibold text-white shadow-lg transition hover:bg-[#20bd5a] sm:w-auto"
            >
              Abrir WhatsApp
            </a>
          </section>
          ...
          <div
            ...
            class="w-full rounded-2xl border border-zinc-800 bg-zinc-900/50 p-5 md:p-6"
          >
            <p class="mb-3 text-xs font-medium uppercase tracking-wide text-zinc-500">Redes e links</p>
          ...
          <section class="w-full rounded-2xl border border-zinc-800 bg-zinc-900/50 p-5 md:p-6">
            ...
            <ul v-if="vm.comments.length > 0" class="mt-8 space-y-6 border-t border-zinc-800 pt-8">
              ...
              >
                ...
                <!-- Resposta do anunciante (logado, dono do perfil) -->
...
const gender = computed(() => route.params.gender as GenderSlug)
const slug = computed(() => route.params.slug as string)

const genders: GenderSlug[] = ['homens', 'mulheres', 'trans']

const { data: apiJson, pending } = await useAsyncData(
  computed(() => `public-advertiser-${route.params.slug as string}`),
  async () => {
    try {
      return await $fetch<ApiPublicAdvertiser>(
        `${config.public.apiBase}/v1/public/advertisers/${route.params.slug as string}`,
      )
    } catch {
      return null
    }
...
watch(
  () => apiJson.value,
  (a) => {
    if (a?.explore_gender_slug && genders.includes(gender.value) && a.explore_gender_slug !== gender.value) {
      navigateTo(`/explorar/${a.explore_gender_slug}/perfil/${slug.value}`, { replace: true })
    }
  },
  { immediate: true },
)
...
function vmFromMock(m: ProfileDetail): ProfileView {
  ...
  return {
    ...
    exploreGenderSlug: gender.value,
    about: m.about,
    serviceLabel,
...
const vm = computed((): ProfileView | null => {
  ...
  const mock = getProfileDetailMock(gender.value, slug.value)
  if (!mock) {
    return null
  }
  return vmFromMock(mock)
})
...
const backLink = computed(() => `/explorar/${vm.value?.exploreGenderSlug ?? gender.value}`)
