<template>
  <div v-if="genderOk && state" class="space-y-8 pb-8">
  ...
  <div v-else class="rounded-xl border border-amber-900/50 bg-amber-950/30 p-6 text-amber-200">
    Estado inválido. <NuxtLink :to="`/explorar/${gender}`" class="underline">Voltar</NuxtLink>
  </div>
</template>

<script setup lang="ts">
import type { GenderSlug } from '~/data/mock-catalog'
import { getStateByGender } from '~/data/mock-catalog'
...
const state = computed(() => {
  ...
  return getStateByGender(gender.value as GenderSlug, uf.value, { showAllCities: true })
})
</script>
